...
class GraRep():
    ...
    def __init__(self, dimensions: int = 32, iteration: int = 10, order: int = 5, seed: int = 42, weight="weight"):
    ...
    def _create_base_matrix(self, graph):
        ...
        """
        A = nx.adjacency_matrix(graph, nodelist=range(graph.number_of_nodes()), weight=self.weight)
    ...
    def get_embedding(self) -> np.array:
        ...
        embedding = np.mean(self._embeddings, axis=0)
        embedding = np.divide(embedding, np.linalg.norm(embedding, axis=1)[:, None])
        return embedding