...
if __name__ == '__main__':
    ...
    graph = utilities.load_edgelist_file_to_dgl_graph(path=input_path, undirected=True, edge_weights=None) #type: dgl.graph
    ...
    embedding_output_path = f"../output/embedding/{file_name}.pkl"
    ...
    landmark_distance_output = f"../output/landmarks_distance/dist_{file_name}.pkl" # where to store the ouput file
    print("Calculating landmarks distance...")
    utilities.calculate_landmarks_distance(landmark_nodes, nx_graph, output_path=landmark_distance_output)
    print("Done landmarks distance!")

    #####  [optional] Plot the network
    ### utilities.plot_nx_graph(nx_graph)

    ##### Step 4: Create datasets to train a model
    ### Option 1. Split data into train/valid sets, write datasets to files

    ### Option 2. Use all data to train, write datasets to files
