...
from evaluations import evaluate_result
...
def run_linear_regression(datasets, use_standard_scaler=False, merge_train_val=True):
    ...
    scores = evaluate_result(y_test, y_pred, print_out=True)
...
def run_neural_net(datasets, file_name):
    ...
    max_dist = math.ceil(
        max(values))  # have questions: why it is a float => because of casting astype(np.float32) when reading `y_train`
    ...
    epoch_wise_scores = []
    for epoch in range(start_epoch, start_epoch + params['epochs']):  # loop over the data_generator.yaml multiple times
        running_loss = 0.0
        stime = time.time()
